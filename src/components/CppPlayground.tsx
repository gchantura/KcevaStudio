...
import { Play, Square, Code, Sliders, Download, Sparkles, Cpu, CheckCircle2, RefreshCw } from 'lucide-react';
...
export function CppPlayground() {
  ...
  const [customPresets, setCustomPresets] = useState<CppDspPreset[]>([]);
 ...
 * Generated by Music Generator Studio (AI Studio Build)
 *
...
`;
  ...
  const handleGenerateAiDsp = async () => {
    if (!aiPrompt.trim()) return;
    setIsGeneratingAi(true);
    try {
      const res = await fetch('/api/generate-cpp-dsp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          prompt: aiPrompt,
          soundType: 'C++ Real-time DSP Synthesizer',
        }),
      });
      const data = await res.json();
      if (data.success && data.dsp) {
        const newPreset: CppDspPreset = {
          id: `custom_${Date.now()}`,
          name: data.dsp.title || 'AI Custom C++ DSP Synth',
          category: 'AI Generated',
          description: data.dsp.description || data.dsp.explanation,
          formula: 'y(t) = \\text{C++ DSP Real-time Function}',
          code: data.dsp.cppCode,
          params: (data.dsp.parameters || []).map((p: any) => ({
            name: p.name,
            label: p.name,
            min: p.min || 0,
            max: p.max || 10,
            step: (p.max - p.min) / 100 || 0.1,
            defaultValue: p.defaultValue || (p.max + p.min) / 2,
            unit: p.unit || '',
          })),
        };
      ...
      console.error('Failed to generate AI C++ DSP:', err);
    } finally {
      setIsGeneratingAi(false);
    }
  };

  const allPresets = [...customPresets, ...CPP_DSP_PRESETS];
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          <div className="bg-slate-900/90 rounded-xl p-4 border border-slate-800 space-y-3">
            <div className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-purple-400">
              ...
              <span>Generate Custom C++ DSP Algorithm</span>
            </div>
            <div className="flex gap-2">
              <input
                type="text"
                placeholder="e.g. Resonant comb filter with stereo chorus..."
                value={aiPrompt}
                onChange={(e) => setAiPrompt(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleGenerateAiDsp()}
              ...
              />
              <button
                id="btn-generate-ai-cpp"
                onClick={handleGenerateAiDsp}
                disabled={isGeneratingAi || !aiPrompt.trim()}
                className="px-3.5 py-2 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white rounded-lg text-xs font-medium transition flex items-center gap-1.5"
              >
